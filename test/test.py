...
Getting Started with SOS LLM Testing:

Prerequisites:

1. Download Our checkpoint: Choose the desired checkpoint and download it.
2. Get the SOS LLM tokenizer: Download the tokenizer file required for your model.
3. Install SOS LLM: Follow the straightforward instructions in the README to install the SOS LLM repository.

Ready to Test!

Here's how to run the test.py script:

python test.py --path_checkpoint=${PATH_TO_THE_SOS_CHECKPOINT} \
    --path_tokenizer=${PATH_TO_THE_SOS_TOKENIZER} \
...
_STRING_TO_SAMPLE = flags.DEFINE_string(
    "string_to_sample",
    "When is the peak trading hour of the day ?",
